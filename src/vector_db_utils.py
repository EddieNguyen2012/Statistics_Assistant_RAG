...
def connect_db(path='./vector_db'):
    ...
    if len(client.list_collections()) == 0:
        client.create_collection('Stat-RAG', embedding_function=SentenceTransformerEmbeddingFunction())
    return client
...
class Database:
    ...
    def create_collection(self, collection_name, embedding_function=SentenceTransformerEmbeddingFunction()):
    ...
    def upsert_docs(self, new_ids, new_docs, new_metadatas, collection):
        collection = self.get_collection(collection)
        collection.upsert(ids=new_ids, documents=new_docs, metadatas=new_metadatas)
